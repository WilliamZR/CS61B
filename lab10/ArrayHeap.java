...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void exchange_nodes(int index1,int index2){
    ...
    private void swim(int index) {
        ...
        while (index>1&&min(index,parentIndex(index))==index){
            ...
            exchange_nodes(parent_index,index);
            index=parent_index;
        }
    }
    ...
    private void sink(int index) {
        // Throws an exception if index is invalid. DON'T CHANGE THIS LINE.
        validateSinkSwimArg(index);
        int sink_index=min(leftIndex(index),rightIndex(index));
        while (index<size&&min(index,sink_index)==sink_index){
            exchange_nodes(sink_index,index);
            index=sink_index;
            sink_index=min(leftIndex(index),rightIndex(index));
        }
    }
    ...
    @Override
    public T peek() {
        Node peek_node=contents[1];
        return peek_node.item();
    }
